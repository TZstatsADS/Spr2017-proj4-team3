...
#    while (TRUE){ # iteratively within E-step to update yh until equilibrium
    ...
    for (i in update_order[1:10]){
        ...
        for (k in 1:K){ # fix all other assignment and just change the assignment for one paper
            ...
            for (Ii in idx_in_cluster_k)
                for (Ij in idx_in_cluster_k)
                    if (Ii != Ij && c2_mtx[Ii,Ij]) 
                        s1 = s1 + weighted_dist(X[Ii,], X[Ij,])
...
#     }
    ...
    s1 = s2 = 0
    for (i in 1:N)
        s1 = s1 + gradient_v(X[i,], Y[x_assign[i],])
    for (i in 1:(N-1)) 
        for (j in i:N) 
