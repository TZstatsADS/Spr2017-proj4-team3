...
coauthor<-function(lis){
  l<-list(0)
  for(i in 1:length(lis)){
    l[[length(l)+1]] <- lis[[i]][[3]]
  }
  return(l)
}

co<-coauthor(data_list[[2]])

cluster<-function(co){
  clust<-list()
  rest<-seq(1, length(co))
  ind<-rest[-length(co)]
  for(i in ind){
    i<-ind[1]
    if (co[[i]]==0||length(co[[i]])==0){
      clust[[length(clust)+1]]<-i
    }
    else{
      same<-c(i)
  ...
  while(continue){
    ...
    for (i in 1:(length(clust)-1)){
      if (length(clust[[i]])!=1){
        same<-c(clust[[i]])
        for(j in (i+1):length(clust)){
          if (sum(clust[[i]]==clust[[j]])>0){
           same<-unique(c(same,clust[[j]]))
           continue<-1
          }
        }
        newclust[[length(newclust)+1]]<-same
      }
      else{
        newclust[[length(newclust)+1]]<-clust[[i]]
      }
    }
    clust<-newclust
  }
